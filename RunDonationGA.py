...
ga = GA.GA(1024, 0.3, 0.3, 10, rand_agents = 500, human_agents= 500, donation = True)
...
GA.pop_v_pop(pop, random + random, 50, donation = True)
...
#run random trials
avg_agent, avg_rand = GA.pop_v_pop(top_half, random, 50, donation = True)
print("Agent Average:", avg_agent, "Random Average:", avg_rand)
avg_agent, avg_human = GA.pop_v_pop(top_half, human, 50, donation = True)
print("Agent Average:", avg_agent, "Human Average:", avg_human)

avg_rand, avg_human = GA.pop_v_pop(random, human, 50, donation = True)
...
plt.legend()
plt.show()
#plt.savefig('DonationFitness1k_2.png') #<-- uncomment out to save (must remove line 41 thought)

#display an agent

print("Best Agent: ", best_agent.tree)
Graph.graphAgent(best_agent, title = "Top Donation Agent", save=False) #<- set save to true to save graph